    ...
    if spec.kind == "edge_cases":
        ...
        while cursor < length - 0.85:
            ...
            if segment_length <= 0.05:
                break
            width = float(rng.uniform(0.48, 1.30))
            center_y = float(np.clip(
            ...
            line_y.append(center_y)
            cursor += segment_length + float(rng.uniform(0.0, 0.14))
        ...
        rectangles = [(0.225, 0.0, 1.05, 1.00)]
        center_y = 0.0
        centers = np.arange(0.95, max(1.0, length - 0.75), 0.42)
        line_x, line_y = [0.20], [0.0]
        for center_x in centers:
            center_y = float(np.clip(
                center_y + rng.uniform(-0.30, 0.30), -0.70, 0.70))
            rectangles.append((float(center_x), center_y,
                               float(rng.uniform(0.30, 0.52)),
                               float(rng.uniform(0.48, 0.90))))
            line_x.append(float(center_x))
            line_y.append(center_y)
        rectangles.append((length - 0.325, center_y, 1.05, 1.00))
        line_x.append(length - 0.25)
        ...
        offset = direction * float(rng.uniform(0.45, 0.85))
        width = float(rng.uniform(0.62, 1.05))
