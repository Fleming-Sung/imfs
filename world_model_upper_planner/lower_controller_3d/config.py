...
ROBOT_ROOT = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "assets")
...
def get_flat_config():
    ...
    cfg["terrain"].update(pillar_size=[0.24, 0.16, 2.0], floor_z=-1.5)
    cfg["foothold"].update(z_distance=[-0.04, 0.04],
                           step_distance=[0.20, 0.32],
                           step_angle_deg=[-10.0, 10.0],
                           movement_direction_deg=[0.0, 0.0],
                           target_yaw_deg=[-6.0, 6.0],
                           hold_probability=0.0, min_swing_distance=0.25,
                           max_rejection_attempts=16)
    cfg["rewards"]["scales"].update(swing_z=6.0, stance_z=3.0)
    cfg["ppo"].update(schedule="fixed", learning_rate=3e-5, entropy_coef=0.002)
