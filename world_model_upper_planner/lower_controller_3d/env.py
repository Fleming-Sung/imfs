...
class FootholdEnv:
    def __init__(self, cfg, sim_params, sim_device, headless=False):
        ...
        # PhysX plane equation n.x + distance = 0.
        plane_params.distance = -float(cfg.terrain.floor_z)
        ...
        # Populate FK before measuring the reset foot offsets; prepare_sim alone
        # leaves derived body tensors at zero on this Isaac Gym version.
        ...
        self.reset_foot_offsets = (self.foot_positions - self.base_position[:, None]).clone()
        self.foot_pillars = torch.tensor([0, 1], device=self.device).repeat(self.num_envs, 1)
        self.target_pillar = torch.full((self.num_envs,), 2, device=self.device, dtype=torch.long)
        self.pillar_target_active = torch.zeros(self.num_envs, device=self.device, dtype=torch.bool)
    ...
    def _create_envs(self):
        ...
        pillar_options.disable_gravity = True
        pillar_asset = self.gym.create_box(self.sim, *cfg.terrain.pillar_size, pillar_options)
        ...
        for i in range(self.num_envs):
            ...
            for j in range(3):
                ...
                pose.p = gymapi.Vec3(float(pos[0]), float(pos[1]) + (j - 1) * 0.3, -3.0)
                ph = self.gym.create_actor(env, pillar_asset, pose, f"support_{j}", i, 0, 0)
                self.gym.set_rigid_body_color(env, ph, 0, gymapi.MESH_VISUAL,
                                             gymapi.Vec3(0.15, 0.55 + 0.1 * j, 0.65))
        ...
        self.num_robot_bodies = self.num_bodies
        self.num_bodies += 3
        self.rigid_body_masses = torch.cat((self.rigid_body_masses,
            torch.zeros(self.num_envs, 3, device=self.device)), dim=1)

    def support_height(self):
        rows = torch.arange(self.num_envs, device=self.device)
        stance = 1 - self.sampler.swing_foot
        pillar = self.foot_pillars[rows, stance] + 1
        return self.root_states[rows, pillar, 2] + 0.5 * self.cfg.terrain.pillar_size[2]

    def _place_target_pillars(self, ids):
        if not len(ids):
            return
        landed_foot = 1 - self.sampler.swing_foot[ids]
        previous = self.pillar_target_active[ids]
        self.foot_pillars[ids[previous], landed_foot[previous]] = self.target_pillar[ids[previous]]
        # Three pillars: retain BOTH occupied supports, relocate only the free one.
        free = 3 - self.foot_pillars[ids].sum(dim=1)
        self.target_pillar[ids] = free
        target = self.sampler.target_pos[ids, self.sampler.swing_foot[ids]]
        self.root_states[ids, free + 1, :3] = target
        self.root_states[ids, free + 1, 2] -= 0.5 * self.cfg.terrain.pillar_size[2]
        self.root_states[ids, free + 1, 3:7] = 0.0
        self.root_states[ids, free + 1, 6] = 1.0
        self.root_states[ids, free + 1, 7:] = 0.0
        actor_ids = (ids * 4 + free + 1).to(torch.int32)
        ...
        self.pillar_target_active[ids] = True
    ...
    def step(self, actions):
        ...
        switched_ids = self.sampler.last_switch_ids
        self._place_target_pillars(switched_ids[~self.goal_reset_pending[switched_ids]])
        self._kick()
    ...
    def _kick(self):
        ...
        self.gym.set_actor_root_state_tensor_indexed(
            ...
            gymtorch.unwrap_tensor((ids * 4).to(torch.int32)), ids.numel())
    ...
    def _post_step(self):
        ...
        if self.goal_reset_pending.any():
            ...
            self.sampler.reset(ids, self.foot_positions,
                               self.rigid_body_states[:, self.feet_indices, 3:7])
            self.pillar_target_active[ids] = False
    ...
    def _check_termination(self):
        ...
        height = self.base_position[:, 2] - self.support_height()
        ...
        # Audit before any reset changes robot state or target.
        self.extras["terminal_root"] = self.root_states[:, 0].clone()
        self.extras["terminal_feet"] = self.foot_positions.clone()
        self.extras["terminal_targets"] = self.sampler.target_pos.clone()
    ...
    def _reset_idx(self, ids):
        ...
        self.foot_pillars[ids] = torch.tensor([0, 1], device=self.device)
        self.target_pillar[ids] = 2
        self.pillar_target_active[ids] = False
        feet = self.root_states[ids, 0, None, :3] + self.reset_foot_offsets[ids]
        for j in range(3):
            self.root_states[ids, j + 1] = 0.0
            self.root_states[ids, j + 1, 6] = 1.0
            self.root_states[ids, j + 1, :2] = feet[:, min(j, 1), :2]
            self.root_states[ids, j + 1, 2] = -0.5 * self.cfg.terrain.pillar_size[2] if j < 2 else -4.0
        pillar_ids = (ids[:, None] * 4 + torch.arange(1, 4, device=self.device)).flatten().to(torch.int32)
        self.gym.set_actor_root_state_tensor_indexed(self.sim, gymtorch.unwrap_tensor(self.root_states),
            gymtorch.unwrap_tensor(pillar_ids), len(pillar_ids))
