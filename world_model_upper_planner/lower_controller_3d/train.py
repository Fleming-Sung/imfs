...
def parse_args():
    custom = [
        ...
        {"name": "--height_range", "type": float, "default": 0.04},
        {"name": "--height_ramp", "type": int, "default": 100},
        {"name": "--iterations", "type": int, "default": 200},
    ]
...
def main():
    ...
    cfg.runner.max_iterations = start_iter + args.iterations
    ppo.learning_rate = float(cfg.ppo.learning_rate)
    for pg in ppo.optimizer.param_groups:
    ...
    for it in range(start_iter, cfg.runner.max_iterations):
        ...
        height = args.height_range * min(1.0, (it - start_iter + 1) / max(args.height_ramp, 1))
        cfg.foothold.z_distance = [-height, height]
        ...
        with open(os.path.join(log_dir, "progress.jsonl"), "a") as stream:
            stream.write(json.dumps(dict(iteration=it, local_iteration=it-start_iter+1,
                elapsed_seconds=elapsed, height_range_m=height, fps=fps,
                lower_steps=(it-start_iter+1)*cfg.env.num_envs*cfg.runner.num_steps_per_env,
                mean_episode_length=float(np.mean(lenbuf)) if lenbuf else None)) + "\n")
