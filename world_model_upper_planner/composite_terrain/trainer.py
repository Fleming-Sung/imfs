        ...
        for step in range(horizon):
            ...
            with torch.no_grad():
                ...
                if "next_action" in batch:
                    # Conservative in-data SARSA bootstrap for fixed replay.
                    # Unexecuted candidates still receive geometry labels, not
                    # invented dynamic values from unrestricted max-Q.
                    target_action=self.target.predict_action(target_next,batch["next_action"][:,step])
                    next_q=target_action["q"].min(0).values*batch["bootstrap_valid"][:,step]
                else:
            ...
            # Auto-reset observations are not physical terminal successors.
            # Keep terminal outcome/Q supervision, but never learn reset jumps.
            live = valid * (1.0 - batch["done"][:, step])
            consistency = (((predicted_next - target_next).square().mean(-1)
                            * live).sum() / live.sum().clamp_min(1.0))
