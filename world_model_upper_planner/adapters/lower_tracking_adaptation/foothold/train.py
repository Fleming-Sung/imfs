"""Isolated PPO continuation for higher-precision lower target tracking.
...
def main():
    ...
    cfg = get_flat_config()
    # The baseline already survives and covers the requested target range. This
    # continuation changes only the reward balance: prioritize realized swing
    # and latched stance XY tracking, while relaxing action smoothness enough
    # to reach long/lateral targets. Domain randomization remains enabled.
    cfg.rewards.scales.swing_xy = 7.0
    cfg.rewards.scales.stance_xy = 7.0
    cfg.ppo.schedule = "fixed"
    cfg.ppo.learning_rate = 3e-5
    cfg.ppo.entropy_coef = 0.002
